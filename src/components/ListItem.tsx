...
const ListItem = ({
	...
	classNames,
	...props
}: ListItemProps & { children: React.ReactNode; classNames?: string }) => {
	return (
	...
	"flex h-28 cursor-pointer flex-row items-center rounded-md  px-4 py-6 transition duration-300 ease-in-out",
	{
		variants: {
			intent: {
				...
				unpaid: "text-yellow-700 bg-yellow-200 hover:bg-yellow-300 dark:text-yellow-200 dark:bg-yellow-700 dark:hover:bg-yellow-800",
			},
		},
	}
);

interface ListItemProps extends VariantProps<typeof listStyles> {}
